...
graph TD
  ...
  M["UDP 多播(Multicast)"]
  I["IPC(共享内存)"]
  R["Aeron 可靠传输层(NAK 重传协议 · 滑动窗口流控 · 位置计数器体系)"]
  T --> U
  T --> M
  T --> I
  U --> R
...
<div class="mermaid">
graph LR
  subgraph LB["Aeron Receiver 的 Log Buffer"]
    direction LR
    A["已处理: seq=1"]
    B["已处理: seq=2"]
    C["可立即处理: seq=4"]
  ...
  A --- B
  C --- D
  N["后台异步 NAK 请求 seq=3 重传"]
  C -.-> N
  D -.-> N
  M["offset 已写入, 可消费(无论 seq=3 是否到达)"]
  C --> M
</div>
...
flowchart LR
  ...
  MDA -->|"UDP Packet (src: 10.0.0.1:9000 / dst: 10.0.0.2:9000)"| MDB
</div>
...
<div class="mermaid">
graph LR
P["发布端 Media Driver(单次发送)"]
SW["交换机(复制转发至所有组成员)"]
B["订阅端 1 App B"]
C["订阅端 2 App C"]
D["订阅端 3 App D"]
P -->|"组播数据包 239.1.1.1:9000"| SW
SW --> B
SW --> C
SW --> D
</div>
...
subgraph Host["同一台主机"]
  PA["Process A (offer())"]
  PB["Process B (poll())"]
  SM["Shared Memory(mmap / /dev/shm) / Log Buffer(Term Buffer x 3)"]
  PA -->|"write(无系统调用)"| SM
  SM -->|read| PB
end
</div>
...
graph TD
  subgraph ACK["ACK 模式(TCP)"]
    ...
    AS4 --> AR4
    AS5 --> AR5
    ANOTE["5 条确认,100% 额外流量"]
    AR3 -.-> ANOTE
  end
  subgraph NAK["NAK 模式(Aeron)"]
    direction LR
    NS1["发送端 包1"]
    NS2["发送端 包2"]
    NS3["发送端 包3(X 丢失)"]
...
<div class="mermaid">
graph TD
  P1["阶段1 正常传输: Sender 发送 [1][2][3][4][5], Receiver 全部确认"]
  P2["阶段2 丢包发生: [3]丢失(X), Receiver 检测到 seq gap(term offset 不连续)"]
  P3["阶段3 NAK 延迟等待(默认 20 μs): 等待乱序帧可能延迟到达, 避免对乱序帧误发 NAK"]
  P4["阶段4 NAK 发送: Receiver 发出 NAK(termId=X, offset=Y), Sender 接收 NAK"]
  P5["阶段5 重传: Sender 重传 [3], Receiver gap 填充, 恢复连续性"]
  P6["阶段6 正常传输继续: Sender 发送 [6][7], Receiver 确认"]
  P1 --> P2
  P2 --> P3
  P3 --> P4
  P4 --> P5
...
graph TD
  ...
  subgraph LB["Log Buffer"]
    C["已消费 — consumerPosition"]
    S["已处理 — subscriberPosition(每个 Subscriber 独立)"]
    R["已接收 — receiverHwm(High Water Mark)"]
    SP["已发送 — senderPosition"]
    PL["可写上限 — publisherLimit(由 Conductor 更新)"]
  end
  P --> LB
  LB --> N
</div>
...
<div class="mermaid">
flowchart TD
A([开始选择传输方式]) --> B{通信双方是否在同一主机?}
B -->|是| C["使用 IPC(最低延迟)"]
B -->|否| D{订阅方是否大于 1 个?}
D -->|是| E{网络是否支持 IP 组播?}
D -->|否| F[UDP 单播]
E -->|是| G["UDP 多播(推荐)"]
E -->|否| H["UDP 单播(多份) 或代理广播"]
</div>
